...
function voteArrayCounter(data, next) {
    $.each(data, function(i, v) {
        ...
        {
            setTimeout(function(){animatevote("like", 417, 582);}, getRandomInt(50, 2500));
        }
        if(v.type == "LOVE" && realtimer["LOVE"] > oldloves)
        {
             setTimeout(function(){animatevote("love", 770, 925);}, getRandomInt(50, 2500));
        }
    });
    if (next) {
        pageLoop(next);
...
function applyVotes() {
    ...
    postToRedis(JSON.stringify(realtimer));
    setTimeout(realTimeReactions, 5000);
}

function postToRedis(theData) {
    dataToPost = JSON.stringify(realtimer);
$.ajax({
            type: "POST",
            url: "API/postresults.php",
