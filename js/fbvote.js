...
function voteArrayCounter(data, next) {
    $.each(data, function(i, v) {
        ...
        {
            setTimeout(function(){animatevote("like", 872, 1037);}, getRandomInt(50, 1000));
        }
        if(v.type == "LOVE" && realtimer["LOVE"] > oldloves)
        {
             setTimeout(function(){animatevote("love", 229, 393);}, getRandomInt(50, 1000));
        }
    });
    if (next) {
        pageLoop(next);
...
function animatevote(type, place1, place2)
{
    ...
    $("#"+dodo).animate({ top: 420}, { duration: 1000, queue: false });
    setTimeout(function(){$("#"+dodo).animate({ opacity: 0},  { duration: 200, queue: false, complete: function(){$("#"+dodo).remove();} });},600);
}